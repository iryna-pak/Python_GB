...
range возвращает полуинтервал. Правую границу не включает. 
Поэтому, чтобы включить значение 240 тоже в наш числовой ряд, нам нужно взять полуинтервал range(20, 241)
"""
print('\n************* Task №4 **************')
...
"""
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!Комментарии Преподавателя!!!!!!!!!!!!!!!!!!!!!!!!!!!
Вот тут обратили внимание, что ваша функция довольна короткая (из одной строчки всего состоит, вот в таких случая
можно и нужно использовать анонимные функции. Они выполняются быстрее).
Вот тут можно в range сразу задать шаг и не задумываться, что элементы должны делиться на 2 без остатка:
range(100,1001, 2)
Тоже же будут четные в указанном диапазоне.
"""
print('\n************* Task №6 **************')
